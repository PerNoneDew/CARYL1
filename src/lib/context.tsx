...
export interface BookingContextType {
  ...
  recordPayment: (bookingId: string, method: PaymentMethod, amount: number, reference?: string, screenshot?: string, staffName?: string) => void;
  recordEventPayment: (eventId: string, method: PaymentMethod, amount: number, reference?: string, screenshot?: string, staffName?: string) => void;
  recordFacilityPayment: (facilityBookingId: string, method: PaymentMethod, amount: number, reference?: string, screenshot?: string, staffName?: string) => void;
  ...
  isLoading: boolean;
  functionHallPhotos: string[];
  addFunctionHallPhoto: (image: string) => Promise<void>;
  deleteFunctionHallPhoto: (index: number) => Promise<void>;
}
...
const dbFacilityBookingToFacilityBooking = (db: DatabaseFacilityBooking): FacilityBooking => ({
  ...
  acceptedByStaff: (db as { accepted_by_staff?: string | null }).accepted_by_staff || null,
...
export function BookingProvider({ children }: { children: ReactNode }) {
  ...
  const [functionHallPhotos, setFunctionHallPhotos] = useState<string[]>([]);
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const [
          ...
          supabase.from('function_hall_photos').select('*').order('sort_order', { ascending: true }),
        ]);
        ...
        // Set function hall photos
        if (hallPhotosData.data) {
          setFunctionHallPhotos((hallPhotosData.data as { image_url: string }[]).map((p) => p.image_url));
        }
        ...
        if (appSettingsData.data && appSettingsData.data.length > 0) {
          ...
          setBusinessInfoState({
            ...
            functionHallTables: (settings as DatabaseAppSettings & { function_hall_tables?: number }).function_hall_tables ?? 0,
            functionHallChairs: (settings as DatabaseAppSettings & { function_hall_chairs?: number }).function_hall_chairs ?? 0,
            functionHallScheduleStart: (settings as DatabaseAppSettings & { function_hall_schedule_start?: string }).function_hall_schedule_start || undefined,
            functionHallScheduleEnd: (settings as DatabaseAppSettings & { function_hall_schedule_end?: string }).function_hall_schedule_end || undefined,
          });
          if (hallPhotosData.data) {
            setFunctionHallPhotos((hallPhotosData.data as { image_url: string }[]).map((p) => p.image_url));
          }
  ...
  const updateBooking = async (id: string, updates: Partial<Booking>) => {
    try {
      ...
      if (updates.acceptedByStaff !== undefined) dbUpdates.accepted_by_staff = updates.acceptedByStaff || null;
  ...
  const addCottage = async (cottage: Cottage) => {
    try {
      ...
      if (error) throw error;
      if (data) {
        const newCottage = dbCottageToCottage(data as DatabaseCottage);
  ...
  // Payment methods
  const recordPayment = async (bookingId: string, method: PaymentMethod, _amount: number, reference?: string, screenshot?: string, staffName?: string) => {
    await updateBooking(bookingId, {
      ...
      acceptedByStaff: staffName || null,
    });
  };

  const recordEventPayment = async (eventId: string, method: PaymentMethod, _amount: number, reference?: string, screenshot?: string, staffName?: string) => {
  ...
  const recordFacilityPayment = async (facilityBookingId: string, method: PaymentMethod, _amount: number, reference?: string, screenshot?: string, staffName?: string) => {
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          function_hall_tables: info.functionHallTables ?? 0,
          function_hall_chairs: info.functionHallChairs ?? 0,
          function_hall_schedule_start: info.functionHallScheduleStart || null,
          function_hall_schedule_end: info.functionHallScheduleEnd || null,
        })
        .neq('id', '00000000-0000-0000-0000-000000000000');
  ...
  const handleAddFunctionHallPhoto = async (image: string) => {
    const { data, error } = await supabase
      .from('function_hall_photos')
      .insert({ image_url: image, sort_order: functionHallPhotos.length })
      .select()
      .single();
    if (error) throw error;
    if (data) setFunctionHallPhotos((prev) => [...prev, (data as { image_url: string }).image_url]);
  };

  const handleDeleteFunctionHallPhoto = async (index: number) => {
    const photo = functionHallPhotos[index];
    if (!photo) return;
    const { error } = await supabase
      .from('function_hall_photos')
      .delete()
      .eq('image_url', photo);
    if (error) throw error;
    setFunctionHallPhotos((prev) => prev.filter((_, i) => i !== index));
  };
  ...
  const value: BookingContextType = {
    ...
    isLoading,
    functionHallPhotos,
    addFunctionHallPhoto: handleAddFunctionHallPhoto,
    deleteFunctionHallPhoto: handleDeleteFunctionHallPhoto,
  };
