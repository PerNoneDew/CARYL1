...
export function EventBookingModal({
  ...
  const { eventTypePrices, eventBookings } = useBooking();
  ...
  const handleConfirm = () => {
    ...
    const conflict = eventBookings.find(
      (b) =>
        b.status !== 'cancelled' &&
        b.status !== 'completed' &&
        new Date(eventDate) < new Date(b.eventEndDate) &&
        new Date(b.eventDate) < new Date(eventEndDate)
    );
    if (conflict) {
      alert(`BOOKING CONFLICT!\n\nThe Function Hall is already booked from ${conflict.eventDate} to ${conflict.eventEndDate}.\n\nPlease select different dates.`);
      return;
    }
