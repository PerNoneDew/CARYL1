...
import { CheckCircle, XCircle, Clock, Printer, Eye, X, User, Home } from 'lucide-react';
...
interface UnifiedPayment {
  ...
  reference?: string;
  screenshot?: string;
  acceptedByStaff?: string | null;
}

export default function PaymentsPage() {
  const navigate = useNavigate();
  const { bookings, eventBookings, facilityBookings } = useBooking();
  ...
  const payments = useMemo<UnifiedPayment[]>(() => {
    const roomPayments: UnifiedPayment[] = bookings.filter((b) => b.bookingType === 'room').map((b) => ({
      id: b.id,
      type: 'room' as const,
      ...
      method: b.paymentMethod === 'gcash' ? 'GCash Payment' : b.paymentMethod === 'maya' ? 'Maya Payment' : b.paymentMethod === 'counter' ? 'Counter Payment' : 'Counter Payment',
      reference: b.paymentReference,
      screenshot: b.transactionScreenshot,
      acceptedByStaff: b.acceptedByStaff,
    }));

    const cottagePayments: UnifiedPayment[] = bookings.filter((b) => b.bookingType === 'cottage').map((b) => ({
      id: b.id,
    ...
    return [...roomPayments, ...cottagePayments, ...eventPayments, ...facilityPayments];
  }, [bookings, eventBookings, facilityBookings]);

  const handleGenerateReceipt = (payment: UnifiedPayment) => {
    const booking = (payment.type === 'room' || payment.type === 'cottage')
  ...
  const getTypeBadge = (type: PaymentType) => {
    switch (type) {
      ...
      case 'cottage':
        return <Badge className="bg-green-100 text-green-800">Cottage</Badge>;
      case 'event':
            ...
            <div className="bg-white rounded-lg p-4 mb-6 border border-gray-200 flex flex-wrap gap-4 items-end">
              <div>
                ...
                >
                  ...
                  <option value="cottage">Cottage Bookings</option>
                      ...
                      <tbody>
                        {filteredPayments.map((payment) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-gray-700">
                              {payment.method !== 'Counter Payment' && payment.screenshot ? (
                                <button
