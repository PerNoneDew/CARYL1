...
export default function LandingPage() {
  ...
  const { isAuthenticated, currentUser, logout, rooms, services, cottages, foodMenuItems, businessInfo, functionHallPhotos } = useBooking();
        ...
        <div className="max-w-7xl mx-auto px-6 py-16">
          ...
          {visibleCategories.length === 0 && availableCottages.length === 0 && availableFoodItems.length === 0 ? (
            ...
            <div className="space-y-14">
              ...
              {/* ── Function Hall ── */}
              {(servicesByCategory['function-hall'].length > 0 || functionHallPhotos.length > 0) && (
                <CategorySection
                  title="Function Hall"
                  icon={CATEGORY_CONFIG['function-hall'].icon}
                  count={servicesByCategory['function-hall'].length || functionHallPhotos.length}
                >
                  {functionHallPhotos.length > 0 && (
                    <div className="mb-6">
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {functionHallPhotos.map((photo, idx) => (
                          <div key={idx} className="overflow-hidden rounded-xl shadow-sm border border-gray-100 group cursor-pointer transition-all duration-300 ease-out hover:-translate-y-2 hover:scale-[1.02] hover:shadow-2xl">
                            <div className="relative h-56 overflow-hidden">
                              <img
                                src={photo}
                                alt={`Function Hall ${idx + 1}`}
                                className="room-card-img w-full h-full object-cover transition-transform duration-500"
                              />
                              {idx === 0 && (
                                <>
