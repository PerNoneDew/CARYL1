...
import { LogIn, Search, Printer, Wallet, AlertCircle } from 'lucide-react';
  ...
  const handleCheckIn = (bookingId: string) => {
    ...
    if (booking.paymentStatus !== 'completed') {
      showErrorNotification({
        title: 'Payment Required Before Check-In',
        description: `Guest ${booking.guestName} has not paid yet. Please accept the payment on the Payments page first.`,
      });
      return;
    }
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex gap-4 mt-2 text-sm text-gray-600">
                                ...
                                {booking.paymentStatus === 'completed' ? (
                                  <Badge className="bg-green-100 text-green-800">Payment Completed</Badge>
                                ) : (
                        ...
                        <div className="flex flex-col items-end gap-2">
                          <div className="flex gap-2">
                            ...
                            {booking.paymentStatus === 'completed' ? (
                              ...
                              <Link
                                to="/staff/payments"
                                className="flex items-center gap-2 px-6 py-3 font-semibold rounded-lg bg-amber-600 hover:bg-amber-700 text-white transition whitespace-nowrap"
                                title="Accept payment before checking in"
                              >
                                <Wallet size={18} />
                          ...
                          {booking.paymentStatus !== 'completed' && (
                            <p className="flex items-center gap-1 text-xs text-amber-600">
                              <AlertCircle size={12} />
                              Payment must be accepted before check-in
                            </p>
                          )}
                        </div>
                      </div>
